...
test.serial('GET /admin returns 404 - authenticated admin user', async t => {
...
test.serial('GET /admin/queues returns admin dashboard - authenticated admin user', async t => {
	const { id, username, avatar } = t.context.user;
	const user = { id, username, avatar };
	const token = generateToken(user);
